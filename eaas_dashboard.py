...
segmentacao_fl = {
    'Segmento': [
        'Startups Growth (R$500k-2M)',
        'PMEs Pequenas (R$500k-2M)',
        'PMEs Micro (<500k)'
    ],
...
with tab2:
    ...
    df_segmentacao = pd.DataFrame(segmentacao_fl)
    st.dataframe(df_segmentacao, use_container_width=True)
    ...
    with col1:
        ...
        fig_dist = go.Figure(data=[
            go.Pie(
                labels=df_segmentacao['Segmento'],
                values=df_segmentacao['Qtd. Estimada'],
    ...
    with col2:
        ...
        fig_ticket_seg = go.Figure(data=[
            go.Bar(
                x=df_segmentacao['Segmento'],
                y=df_segmentacao['Ticket Médio (R$)'],
                marker_color=['#FF6B6B', '#4ECDC4', '#45B7D1']
            )
        ])
        st.plotly_chart(fig_ticket_seg, use_container_width=True)
...
with tab3:
    ...
    segmentacao_sc = {
        'Segmento': ['Startups Growth + Aceleradoras', 'PMEs Médias', 'PMEs Pequenas/Micro'],
    ...
    with col1:
        fig_seg_sc_pie = go.Figure(data=[
            go.Pie(
                labels=df_seg_sc['Segmento'],
                values=df_seg_sc['Qtd. Estimada'],
                marker_colors=['#FF6B6B', '#4ECDC4', '#45B7D1']
            )
        ])
    ...
    with col2:
        ...
        fig_receita_sc = go.Figure(data=[
            go.Bar(
                x=df_seg_sc['Segmento'],
                y=df_seg_sc['Receita Mensal'],
                marker_color=['#FF6B6B', '#4ECDC4', '#45B7D1']
            )
        ])
...
with tab4:
    ...
    segmentacao_br = {
        'Segmento': [
            'Startups Growth (R$500k-2M)',
            'Startups Scale (R$2M-10M)',
            'PMEs Médias (R$500k-2M)',
            'PMEs Pequenas/Micro (<500k)'
        ],
    ...
    with col1:
        ...
        fig_seg_br_pie = go.Figure(data=[
            go.Pie(
                labels=df_seg_br['Segmento'],
                values=df_seg_br['Qtd. Estimada'],
                marker_colors=['#FF6B6B', '#4ECDC4', '#45B7D1', '#F7DC6F']
            )
        ])
    ...
    with col2:
        ...
        fig_receita_br = go.Figure(data=[
            go.Bar(
                x=df_seg_br['Segmento'],
                y=df_seg_br['Receita Mensal (R$)'],
                marker_color=['#FF6B6B', '#4ECDC4', '#45B7D1', '#F7DC6F']
            )
        ])
...
with tab5:
    ...
    with col1:
        ...
        fig_empresas_comp = go.Figure(data=[
            go.Bar(x=df_mercado['Região'], y=df_mercado['Empresas Alvo'], 
                   marker_color=['#FF6B6B', '#4ECDC4', '#45B7D1'])
        ])
    ...
    with col2:
        ...
        fig_mercado_comp = go.Figure(data=[
            go.Bar(x=df_mercado['Região'], y=df_mercado['Mercado (R$ mi)'],
                   marker_color=['#FF6B6B', '#4ECDC4', '#45B7D1'])
        ])
